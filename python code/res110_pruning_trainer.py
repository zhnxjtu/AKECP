...
def main():
    ...
    #######################################
    layer_index = [6, 8, 12, 14, 18, 20, 24, 26, 30, 32, 36, 38, 42, 44, 48, 50, 54, 56, 60, 62, 66, 68,
    ...
    for layer_num in range(1,109):
        # load pruned channel
        d['pruned_filter' + str(layer_num)] = load_pruning_file(layer_num)
        # pruning
        pruning(model, layer_index[int(layer_num) - 1], d['pruned_filter' + str(layer_num)])


    ac1, ac5 = evalute_Top1_and_Top5(model, val_loader, device)
    print('Test Top-1 accuracy: \033[31m {:.6f} \033[0m, Top-5 accuracy: {:.6f}'.format(ac1, ac5))
    ...
    lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer,
                                                        milestones=[30, 70], last_epoch=args.start_epoch - 1)
    ...
    file_name = open('./Res110_acc-4.txt', mode='w')
...
def evalute_Top1_and_Top5(model, loader, device):
    ...
    for data in loader:
        ...
        maxk = max((1, 5))
        y_resize = y.view(-1, 1)
        _, pred1 = logits.topk(maxk, 1, True, True)

        correct_top1 += torch.eq(pred, y).sum().float().item()
        correct_top5 += torch.eq(pred1, y_resize).sum().float().item()
...
def load_pruning_file(layer_num):
    f = open('../matlab code/Res110_PruningIndex/FM' + str(layer_num) + '_1.txt', 'r')
