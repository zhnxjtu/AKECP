function [X, A, B, E,Xconv,alphas, varargout] = VBAS(Y, options,kk)
% AKECP: Adaptive effective knowledge extraction for channel pruning
% -----------------------------------------------------------------------
%                                INPUTS
% Y               : input matrix
...
switch options.init,
    ...
    case 'ml'
        ...
        Sigma_E1 = scale*ones(r,r);
        alphas = ones(m,n)*scale;
        alphas1 = ones(m,n)*scale;
...
%     gammas = ( m+n - m*diag(Sigma_A) - n*diag(Sigma_B) + a_gamma0 )./ (diag(B'*B) + diag(A'*A)+ b_gamma0);
    ...
    %% estimate beta? 
    if  UPDATE_BETA,
        err = sum(sum( abs(Y - X - kk*E).^2 ) ) + n*trace(A'*A*Sigma_B) + m*trace(B'*B*Sigma_A) + m*n*trace(Sigma_A*Sigma_B) + sum(sum(Sigma_E));
